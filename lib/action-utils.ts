...
export async function safeAction<T, Args extends any[]>(
    ...
    try {
        ...
        if (isDatabaseError(error)) {
            return {
                data: null,
                error: "Désolé, le service est temporairement indisponible. Notre base de données ne répond pas. Veuillez réessayer dans quelques instants.",
                status: "error",
            };
        }
 ...
 */
export function withErrorHandler<Args extends any[], T>(
    action: (...args: Args) => Promise<T>
) {
 ...
 */
export function withErrorThrower<Args extends any[], T>(
    action: (...args: Args) => Promise<T>
) {
