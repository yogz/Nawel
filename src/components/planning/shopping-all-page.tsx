...
import { type PlanData, type Item, type Ingredient } from "@/lib/types";
...
export function ShoppingAllPage({
  initialPlan,
  slug,
  writeKey,
  writeEnabled,
}: ShoppingAllPageProps) {
  ...
  // Calculate effective checked state for aggregated items
  const getEffectiveChecked = (aggregatedItem: AggregatedShoppingItem) => {
    return aggregatedItem.sources.every((source) => {
      if (source.type === "ingredient") {
        return source.ingredient?.checked ?? false;
      }
      return source.item.checked ?? false;
    });
  };

  const allItems = useMemo(() => Object.values(shoppingList).flat(), [shoppingList]);
  const checkedCount = allItems.filter((item) => getEffectiveChecked(item)).length;
  ...
  const categoryStats = useMemo(() => {
    ...
    Object.entries(shoppingList).forEach(([category, items]) => {
      const checked = items.filter((item) => getEffectiveChecked(item)).length;
  ...
  const handleToggle = (aggregatedItem: AggregatedShoppingItem) => {
    ...
    startTransition(async () => {
      const newChecked = !getEffectiveChecked(aggregatedItem);
      ...
      <header className="sticky top-0 z-50 border-b border-white/20 bg-white/80 backdrop-blur-sm">
        <div className="mx-auto flex max-w-2xl items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg bg-accent/10">
              <Users size={18} className="text-accent" />
            </div>
            <div>
              <h1 className="text-sm font-semibold text-text">{t("allListTitle")}</h1>
              <p className="text-xs text-muted-foreground">{t("allGuests")}</p>
            </div>
          </div>
