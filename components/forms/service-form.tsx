...
export function ServiceForm({
    ...
    onSubmit: (mealId: number, title: string, peopleCount: number, newMealDate?: string, newMealTitle?: string) => Promise<void>;
    readOnly?: boolean;
}) {
    const [mealId, setMealId] = useState<string>(
        defaultMealId !== undefined && defaultMealId !== -1
            ? String(defaultMealId)
            : (forceNewMeal || meals.length === 0 ? "new" : String(meals[0].id))
    );
                        ...
                        <Popover>
                            <PopoverTrigger asChild>
                                ...
                                >
                                    ...
                                    {newMealDate ? format(newMealDate, "PPP", { locale: fr }) : <span>Choisir une date</span>}
                                </Button>
                            </PopoverTrigger>
